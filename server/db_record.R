...
record_project <- function(db, name, comments) {
  query <- sprintf(
    "insert into project (name, comments, creation, modified)
...
record_sample <- function(db, project, sample_name, sample_id, filepath,
  ...
  print(infos)
  raw_data <- blob::blob(fst::compress_fst(
    serialize(
  ...
  query <- sprintf(
    "insert into sample (sample, raw_data, raw_path, polarity,
  ...
  if (params$chemical_type == "Standard") {
    data <- paste(sapply(1:length(params$standard_type), function(i) {
      paste(sprintf(
        ...
        params$retention_time[[i]][2], params$missing_scans
      ), collapse = ",")
    }), collapse = ",")
...
record_features <- function(db, features) {
  ...
  if (c("no ROIs") %in% features[, "iso"]) {
    ...
    query <- sprintf(
      "insert into feature (mz, mzmin, mzmax, rt, rtmin, rtmax,
