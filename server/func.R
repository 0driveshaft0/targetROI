...
#' @examples
#' \dontrun{
#' db_get_query(db, "select * from mtcars;")
#' }
...
#' @examples
#' \dontrun{
#' db_execute(db, "insert into mtcars (id) values (1);")
#' }
db_execute <- function(db, query, ...) {
  if (length(query) == 0) {
    stop("error when inserting in database")
...
#' @examples
#' \dontrun{
#' toastr_error("Aaaaaaargh", "A zombie bite me...")
#' }
...
#' @examples
#' \dontrun{
#' toastr_success("yeeeeees", "Just dodged a zombie...")
#' }
...
#' @examples
#' \dontrun{
#' toastr_error("Ohoh", "Is it a zombie ?...")
#' }
...
#' @examples
#' \dontrun{
#' sweet_alert_error("Garrrrrrrr", "Braiiiinn...")
#' }
