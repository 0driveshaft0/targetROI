...
overlap_peaks <- function(peaks, eic, baseline, noise) {
  ...
  for (i in seq(nrow(peaks))) {
    ...
    if (length(ids) == 0) {
      peaks_final <- rbind(peaks_final, peaks[i, ])
    } else {
      peaks_final[ids[1], ] <- merge_peaks(
...
merge_peaks <- function(peaks, eic, baseline, noise) {
  ...
  new_peak <- data.frame(
    ...
    sn = if (noise == 0) {
      ...
      intb / pracma::trapz(rep(noise, diff(lm) + 1))
    },
    scale = peaks[1, "scale"],
    scpos = peaks[1, "scpos"],
...
integrate <- function(eic, scalerange, baseline, noise, missing_scans, mzmat) {
  ...
  if (all(sapply(wCoefs, function(x) all(x - baseline < noise)))) {
    return(NULL)
  }
  # get ridge lines
  ...
  overlap_peaks(unique(do.call(rbind, lapply(rL, function(opp) {
    ...
    if (length(lm) < 2) {
      return(NULL)
    } else if (lm[2] - lm[1] < scales[1]) {
      return(NULL)
    }
    integrate2(
      eic, lm, baseline, noise, missing_scans, mzmat,
      best.scale
    )
  }))), eic, baseline, noise)
}
...
integrate2 <- function(eic, lm, baseline, noise, missing_scans, mzmat, scale = NA) {
  ...
  lm <- narrow_rt_boundaries_extend(
    lm, center,
    eic[, "int"] - baseline, missing_scans
  )
  lm <- narrow_rt_boundaries_reduce(
    lm, center,
    eic[, "int"] - baseline, missing_scans
  )
  if (diff(lm) <= 0) {
...
deconvolution <- function(xr, theoric_patterns, chemical_ids, scalerange, scanrange = NULL,
  ...
  for (i in seq(theoric_patterns)) {
    ...
    if (reintegration) {
      ...
      basepeaks <- integrate2(
        traces[[1]]$eic[roi[1]:roi[2], ], lm,
      ...
      basepeaks <- integrate(
        traces[[1]]$eic[roi[1]:roi[2], ], scalerange,
        baselines[[1]][roi[1]:roi[2]], noises[1], missing_scans,
        traces[[1]]$mzmat[
          which(
    ...
    while (k < length(traces) & continue_integration) {
      ...
      peak <- integrate2(
        ...
        mzmat[which(mzmat[, "scan"] %in% roi[1]:roi[2]), , drop = FALSE]
      )[1, ]
      if (length(peak) > 0) {
