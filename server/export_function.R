  ...
  for (chem in chem_type) {
    for (adduct in adducts) {
      ...
      if (adduct %in% deconvolution_params()[which(deconvolution_params()$chemical_type == chem), "adduct"]) {
        ...
        openxlsx::writeData(wb, 2, paste0(mean(c(
          stdInfo$retention_time_min[which(stdInfo$chemical_type == std[1])],
          stdInfo$retention_time_max[which(stdInfo$chemical_type == std[1])]
        )), " min"),
        startCol = 3, startRow = 18
        )
        # Standard RT 2 (given by user)
        openxlsx::writeData(wb, 2, paste0(mean(c(
          stdInfo$retention_time_min[which(stdInfo$chemical_type == std[2])],
          stdInfo$retention_time_max[which(stdInfo$chemical_type == std[2])]
        )), " min"),
