...
output$graphics_bubble_plot <- plotly::renderPlotly({
  ...
  if (!params$active) {
    return(plot_empty_plot("Bubble plot"))
  }
  if (length(params$project_sample) == 0) {
    return(plot_empty_plot("Bubble plot"))
  }
  plot_bubble_plot(
    db, params$project_sample, params$adduct, params$chemical,
...
output$graphics_surface_plot <- plotly::renderPlotly({
  ...
  if (!params$active) {
    return(plot_empty_3d_plot("Surface plot"))
  }
...
output$graphics_contours_plot <- renderPlotly({
  ...
  if (!params$active) {
    return(plot_empty_plot("Contour plot"))
  }
  if (length(params$project_sample) == 0) {
    return(plot_empty_plot("Contour plot"))
  }
  plot_contour_plot(
    db, params$project_sample, params$adduct,
...
output$graphics_histogram_plot <- renderPlotly({
  ...
  if (!params$active) {
    return(plot_empty_3d_plot("3D histogram"))
  }
