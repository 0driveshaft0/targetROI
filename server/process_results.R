...
output$process_results_profile <- DT::renderDataTable(
  ...
  options = list(
    info = FALSE, paging = FALSE, dom = "Bfrtip", scoller = TRUE,
...
output$process_results_standard_table <- DT::renderDataTable(
  {
    ...
    query <- sprintf(
      'select chemical_type, adduct from deconvolution_param where project == %s and
...
observeEvent(input$process_results_standard_selected, {
  tryCatch(
    {
      if (is.null(input$process_results_standard_selected)) {
        custom_stop(
...
output$process_results_eic <- plotly::renderPlotly({
  ...
  tryCatch(
    {
      if (is.null(input$process_results_profile_selected) &
        is.null(input$process_results_standard_selected)) {
        custom_stop(
          "invalid", "no cell selected"
        )
      }
      ...
      params <- list(
        ...
        adduct = if (study != "Standard") {
          isolate(input$process_results_chemical_adduct)
        } else {
          input$process_results_adduct_selected
        },
        chemical_type = if (study != "Standard") {
          isolate(input$process_results_study)
        } else {
          study
        },
        C = as.numeric(input$process_results_profile_selected$C),
      ...
      deconvolution_param <- if (study != "Standard") {
        as.list(deconvolution_params()[which(
      ...
      p <- plot_chemical_EIC(db, params$project_sample, params$adduct,
        ...
        retention_time = c(
          deconvolution_param$retention_time_min,
  ...
  tryCatch(
    {
      ...
      plot_chemical_MS(db, params$project_sample, params$adduct,
        params$chemical_type, params$C, params$Cl, params$formula,
        resolution = list(
          resolution = deconvolution_param$resolution,
          mz = deconvolution_param$resolution_mz,
          index = deconvolution_param$resolution_index
  ...
  tryCatch(
    {
      ...
      if (input$export_format == "Excel") {
        ...
        if (length(c(grep("Chlorinated paraffins", chem_type$chemical_familly), grep("Brominated paraffins", chem_type$chemical_familly))) > 0) {
          ...
          export_PCA(actual_user,
            maxBar = length(unique(chem_type$chemical_familly)) * length(adducts), chem_type = chem_type[c(grep("Chlorinated paraffins", chem_type$chemical_familly), grep("Brominated paraffins", chem_type$chemical_familly)), "chemical_type"],
        ...
        if (length(grep("Chlorinated olefins", chem_type$chemical_familly)) > 0) {
          ...
          adducts <- unique(adducts[which(adducts$project == input$project), "adduct"])
          export_PCO(actual_user,
            maxBar = length(unique(chem_type$chemical_familly)) * length(adducts), chem_type = chem_type[grep("Chlorinated olefins", chem_type$chemical_familly), "chemical_type"],
            adducts = adducts, actual_project_informations, pbValue
          )
        ...
        if (length(grep("Mixed paraffins", chem_type$chemical_familly)) > 0) {
          ...
          adducts <- unique(adducts[which(adducts$project == input$project), "adduct"])
          export_PXA(actual_user,
            maxBar = length(unique(chem_type$chemical_familly)) * length(adducts), chem_type = chem_type[grep("Mixed paraffins", chem_type$chemical_familly), "chemical_type"],
            adducts = adducts, actual_project_informations, pbValue
          )
        ...
        if (length(grep("Phase I metabolites", chem_type$chemical_familly)) > 0) {
          ...
          adducts <- unique(adducts[which(adducts$project == input$project), "adduct"])
          export_phase1(actual_user,
            maxBar = length(unique(chem_type$chemical_familly)) * length(adducts), chem_type = chem_type[grep("Phase I metabolites", chem_type$chemical_familly), "chemical_type"],
            adducts = adducts, actual_project_informations, pbValue
          )
        ...
        if (length(grep("Phase II metabolites", chem_type$chemical_familly)) > 0) {
          ...
          adducts <- unique(adducts[which(adducts$project == input$project), "adduct"])
          export_phase2(actual_user,
            maxBar = length(unique(chem_type$chemical_familly)) * length(adducts), chem_type = chem_type[grep("Phase II metabolites", chem_type$chemical_familly), "chemical_type"],
            adducts = adducts, actual_project_informations, pbValue
          )
...
shiny::observeEvent(input$process_results_reintegration, {
  ...
  params <- list(
    ...
    adduct = input$process_results_chemical_adduct,
    retention_time = c(
      input$process_results_reintegration_rt_min,
  ...
  deconvolution_params <- get_deconvolution_params(
    db, params$project,
  ...
  params$sample <- project_samples()[which(
    project_samples()$project == params$project &
      project_samples()$project_sample == params$project_sample
  ), "sample"]

  tryCatch(
    {
      ion_form <- get_chemical_ion(
        db, params$adduct, params$chemical_type,
        params$C, params$Cl
      )
      if (nrow(ion_form) == 0) custom_stop("minor_error", "no chemical founded
      with this adduct")

      theoric_pattern <- get_theoric(
        ion_form$ion_formula,
      ...
      peak <- deconvolution(ms_file, theoric_pattern, ion_form$chemical_ion, c(1, scalerange[2]),
        params$retention_time, params$missing_scans,
        pb = "pb2", reintegration = TRUE
      )
      ...
      val <- paste(round(peak$score[1]),
        ...
        round(peak$weighted_deviations[1] * 10**3, digits = 1),
        sep = "/"
      )
