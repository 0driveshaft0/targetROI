...
plot_TIC <- function(db, project = NULL, project_samples = NULL,
  ...
  p <- plot_empty_chromato(
    title = if (length(unique(datas$sample_name)) > 1) {
...
plot_chemical_EIC <- function(db, project_sample = NULL,
  ...
  theoric_pattern <- get_theoric(
    chemical_ion$ion_formula,
    chemical_ion$charge, resolution
  )[[1]]
  if (nrow(theoric_pattern) == 0) {
    return(p)
  }
  ...
  # get integrated data
  features <- get_chemical_features(
    db, project_sample,
...
plot_MS <- function(db, project = NULL, project_samples = NULL, rt) {
  ...
  p <- plot_empty_MS(
    title = if (length(unique(datas$sample_name)) > 1) {
      ...
      sprintf("Mass spectrum (%s min)", round(rt, 2))
    }
  )
  if (nrow(datas) == 0) {
    return(p)
...
plot_bubble_plot <- function(
  ...
  data_studied <- if (data_studied == "Intensity (xE6)") {
    "intensities"
  } else if (data_studied == "Score(%)") {
    "score"
  } else {
    "weighted_deviation"
  }
  ...
  fig <- plotly::plot_ly(data2,
    x = ~C, y = ~Cl, text = ~studied, type = "scatter", mode = "markers",
...
#'
#' @return plotly object
plot_contour_plot <- function(
    db, project_sample = NULL, adduct = NULL,
    chemical = NULL, reference = NULL, value = NULL, data_studied = NULL) {
  data_studied1 <- if (data_studied == "Intensities") {
    "intensities"
  } else if (data_studied == "Scores") {
    "score"
  } else {
    "weighted_deviation"
  }
  ...
  if (reference == "Total intensity") {
    ...
    if (data_studied1 == "intensities") {
      limite <- value * 10**6
    } else if (data_studied1 == "weighted_deviation") {
      limite <- value * 10**(-3)
    } else {
      limite <- value
    }
  ...
  fig <- plot_ly(
    x = ~C, y = ~Cl, z = ~Intensity, type = "contour",
...
#'
#' @return plotly object
plot_surface_plot <- function(
    db, project_sample = NULL, adduct = NULL,
    chemical = NULL, data_studied = NULL) {
  data_studied <- if (data_studied == "Intensities") {
    2
  } else if (data_studied == "Scores") {
    1
  } else {
