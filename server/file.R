      ...
      if (nrow(to_add) > 0) {
        ...
        if (nrow(files_in_project) > nrow(to_delete)) {
          project_polarity <- samples()[which(
            samples()$sample %in% project_samples()[
              which(
          ...
          to_add_polarities <- samples()[which(
            samples()$sample %in% to_add$sample
          ), "polarity"]
          test_polarities <- which(to_add_polarities != project_polarity)
          if (any(test_polarities)) {
            custom_stop("invalid", sprintf(
...
output$file_associate_table <- DT::renderDataTable(
    ...
    tryCatch(
      {
        ...
        data.frame(
          ...
          Label = paste('<input type=\"text\" value=\"',
            params$sample_ids,
            '\" maxlength=30 width=\"100%\" required>',
            sep = ""
          )
        )
      },
  ...
  extensions = "Scroller",
  options = list(
    dom = "frtip", fixedColumns = TRUE, bFilter = FALSE, paging = FALSE,
  ...
  tryCatch(
    {
      sapply(1:length(params$sample_names), function(i) {
        record_project_sample(
          db, params$project,
          params$sample_names[i], params$sample_ids[i]
        )
      })
      toastr_success(sprintf(
        "%s imported in sequence %s",
...
shinyFiles::shinyFileChoose(input, "file_import",
  roots = volumes,
...
shiny::observeEvent(input$file_import, {
  ...
  if (is.integer(input$file_import)) {
    return()
  }
  ...
  tryCatch(
    {
      if (is.null(input$project)) {
        custom_stop("invalid", "You must create a sequence
    ...
    tryCatch(
      {
        ...
        data.frame(
          ...
          Label = paste('<input type=\"text\" value=\"',
            ...
            '\" maxlength=30 width=\"100%\" pattern="[^:\'[\\]?\\/\\\\*]+" required/>',
            sep = ""
          )
        )
      },
      invalid = function(i) {
        data.frame(matrix(, nrow = 0, ncol = 2, dimnames = list(
          c(),
          c("File", "Label")
        )), check.names = FALSE)
      },
  ...
  tryCatch(
    {
        ...
        success[i] <- tryCatch(
          {
            ...
            } else if (params$sample_names[i] %in% project_samples()$sample) {
              record_project_sample(
                db, params$project, params$sample_names[i], params$sample_ids[i]
              )
            } else {
              conversion(
                db, params$project, params$sample_names[i],
      ...
      share_vars$file_import_success <- data.frame(
        ...
        Import = sapply(success, function(x) {
          if (x == "success") {
            paste(
              "<div style=\"background-color: #B3E2CD;\">",
              shiny::icon("check-circle"), x, "</div>"
            )
          } else {
            paste(
              "<div style=\"background-color: #FDCDAC;\">",
      ...
      shiny::showModal(shiny::modalDialog(
        title = "Result of import", easyClose = TRUE,
...
conversion <- function(db, project, sample_name, sample_id, filepath, filename, polarity) {
  ...
  if (grepl("WIFF$", filepath, ignore.case = TRUE) &
    !file.exists(paste0(filepath, ".scan"))) {
    stop(
  ...
  algorithm <- if (
    (grepl("raw$", filepath, ignore.case = TRUE) &
      ...
      grepl("mzML$", filepath, ignore.case = TRUE)) {
    "vendor"
  } #' cwt' else 'vendor' cwt only for waters but raw is also thermo

  # call msConvert
  query <- sprintf(
    "\"%s\" \"%s\" -o \"%s\" --outfile \"%s\" --mzXML --64 --zlib --filter \"peakPicking %s msLevel=1\" --filter \"polarity %s\"",
  ...
  if (!file.exists(out_filepath)) {
    ...
    if (grepl("mzXML$", filepath, ignore.case = TRUE) |
      ...
      stop(msconvert_blabla[length(msconvert_blabla)])
    }
  }
  ...
  importation(
    db, project, sample_name, sample_id, out_filepath, filepath,
...
importation <- function(db, project, sample_name, sample_id, filepath, old_filepath,
  ...
  real_polarity <- if (grepl("cdf$", filepath, ignore.case = TRUE)) {
    polarity
  } else {
    unique(MSnbase::polarity(ms_file))
  }
  print(sprintf("detect polarity %s", paste(real_polarity, collapse = ", ")))

  if (is.na(real_polarity)) real_polarity <- polarity
  if (length(real_polarity) > 1) {
    importation(
      db, project, sample_name, sample_id,
      cut_ms_file(ms_file, polarity), old_filepath, polarity, thermo_file
    )
  } else if (polarity == 1 & real_polarity < 1) {
    stop("no positive scans detected")
  } else if (polarity == 0 & real_polarity > 0) {
    stop("no negative scans detected")
  } else {
    record_sample(
      db, project, sample_name, sample_id, filepath, old_filepath, polarity,
      ms_file, thermo_file
    )
  }
}
...
get_ms_file_infos <- function(ms_file, thermo_file = NULL) {
  ...
  infos <- list(
    ...
    maximum_it = thermo_file[which(grepl(
      "Maximum I(njection)?[[:space:]]?T(ime)?", thermo_file
    ))][1],
    number_of_scan_range = thermo_file[which(grepl(
      "Number of scan ranges", thermo_file
    ))][1],
  ...
  if (!is.null(infos$resolution)) {
    infos$resolution <- stringr::str_extract(
      infos$resolution, "[[:digit:]]+([.,EeKk][[:digit:]]*)?"
    )
  }
  if (!is.null(infos$agc_target)) {
    infos$agc_target <- stringr::str_extract(
      infos$agc_target, "[[:digit:]]+([.,EeKk][[:digit:]]*)?"
    )
  }
  if (!is.null(infos$maximum_it)) {
    unit_maximum_it <- stringr::str_extract(
      infos$maximum_it,
      "(([[:alpha:]]+$)|((?<=\\()[[:alpha:]]+(?=\\))))"
    )
    infos$maximum_it <- paste(
      stringr::str_extract(infos$maximum_it, "[[:digit:]]+"),
      unit_maximum_it
    )
  }
  if (!is.null(infos$number_of_scan_range)) {
    infos$number_of_scan_range <- stringr::str_extract(
      infos$number_of_scan_range, "[[:digit:]]+([.,EeKk][[:digit:]]*)?"
    )
  }
  if (!is.null(infos$scan_range)) {
    infos$scan_range <- paste(paste(
