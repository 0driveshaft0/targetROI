  ...
  tryCatch(
    {
      ...
      inputs <- c(
        "regression_observed_file", "regression_observed_family",
        "regression_observed_adduct"
      )
      conditions <- c(
        length(params_observed$file) > 0, length(params_observed$family) > 0,
        length(params_observed$adduct) > 0
      )
      messages <- c(
        "An observed file must be chosen", "A family must be chosen",
        "An adduct must be chosen"
      )
      check_inputs(inputs, conditions, messages)

      inputs <- c(
        "regression_theoretic_files", "regression_theoretic_family",
        "regression_theoretic_adduct"
      )
      conditions <- c(
        length(params_theoretic$files) > 1, length(params_theoretic$family) > 0,
        length(params_theoretic$adduct > 0)
      )
      messages <- c(
        "At least two theoretical files must be chosen", "A family must be chosen",
        "An adduct must be chosen"
      )
      check_inputs(inputs, conditions, messages)
      ...
      for (i in 1:length(params_theoretic$files)) {
        test <- get_features(
          db, params_theoretic$files[i], params_theoretic$adduct,
      ...
      mat_observed <- get_profile_matrix(db, params_observed$file, params_observed$adduct,
        params_observed$family,
        simplify = FALSE, table = TRUE
      )
      ...
      finale_matrix <- matrix(NA,
        nrow = C[2] - C[1] + 1, ncol = Cl[2] - Cl[1] + 1,
      ...
      for (row in seq(nrow(theoretic_mat))) {
        data <- if (theoretic_mat[row] != 0 & observed_norm[row] != 0) {
          paste(round(observed_norm[row] * 10**3, digits = 3),
            round(theoretic_mat[row] * 10**3, digits = 3),
            sep = "/"
          )
        } else if (theoretic_mat[row] != 0) {
          paste("NA",
            round(theoretic_mat[row] * 10**3, digits = 3),
            sep = "/"
          )
        } else if (observed_norm[row] != 0) {
          paste(round(
            observed_norm[row] * 10**3,
            digits = 3
          ), "NA", sep = "/")
        } else {
          paste("NA", "NA", sep = "/")
        ...
        finale_matrix[
          mat_observed[row, "C"] - C[1] + 1,
          mat_observed[row, "Cl"] - Cl[1] + 1
        ] <- data
      }

      files <- project_samples()[
        which(project_samples()$project_sample %in% params_theoretic$files),
...
output$regression_matrix <- DT::renderDataTable(
  ...
  options = list(
    info = FALSE, paging = FALSE, dom = "frtip", scoller = TRUE,
