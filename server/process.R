...
shiny::observeEvent(input$process_instrument, {
  ...
  if (params$instrument == "Orbitrap") {
    ...
    choices <- if (params$instrument == "QTOF_XevoG2-S") {
      setNames(25, "25k@200")
    } else if (params$instrument == "Sciex_TripleTOF5600") {
      setNames(26, "25k@200")
    } else if (params$instrument == "Sciex_TripleTOF6600") {
      setNames(27, "25k@200")
    } else if (params$instrument == "Sciex_QTOFX500R") {
      setNames(28, "25k@200")
    } else {
      setNames(29:31, c(
        "low_extended_highSens_R25000@200",
  ...
  tryCatch(
    {
      if (is.null(params$project)) {
        custom_stop("invalid", "No sequence")
      } else if (params$project == "") custom_stop("invalid", "No sequence")
      htmlwidgets::onRender(
        plot_TIC(db, project = params$project),
        "function(el, x){
  ...
  tryCatch(
    {
      ...
      if (is.null(params$rt)) {
        custom_stop("invalid", "No rt selected")
  ...
  tryCatch(
    {
      ...
      inputs <- c(
        "process_resolution", "process_resolution_mz",
      ...
      msgs <- c(
        "the resolution of instrument is required",
      ...
      messages <- c(
        "the resolution of instrument must be over 0",
      ...
      if (param$standard_study) {
        ...
        conditions <- c(
          length(params_standard$standard_type) > 0,
      ...
      if (param$standard_study) {
        params_standard$retention_time <- lapply(
      ...
      theoric_patterns <- get_theoric(
        ion_forms$ion_formula,
        ...
        theoric_patterns_standard <-
          lapply(1:length(ion_forms_standard), function(i) {
            if (nrow(ion_forms_standard[[i]]) > 0) {
              get_theoric(
                ion_forms_standard[[i]]$ion_formula,
      ...
      for (i in 1:length(params$samples)) {
        ...
        peaks2 <- if (length(deleted) != 0) {
          deconvolution(ms_file, theoric_patterns[-deleted],
            ion_forms[-deleted, ]$chemical_ion, scalerange, params$retention_time,
            params$missing_scans,
            pb = "pb2"
          )
        } else {
          deconvolution(ms_file, theoric_patterns,
        ...
        if (length(peaks2) > 0) {
          ...
          toastr_error(sprintf("No chemical detected	in %s", params$samples[i]))
        }
        ...
        if (param$standard_study) {
          peaks2_standard <- do.call(rbind, lapply(1:length(theoric_patterns_standard), function(i) {
            if (theoric_patterns_standard[[i]] == "no theoric") {
              ...
              notheoric <- c(
                mz = NA, mzmin = NA, mzmax = NA, rt = NA, rtmin = NA, rtmax = NA, into = NA, intb = NA, maxo = NA, sn = NA, scale = NA,
          ...
          if (length(peaks2_standard) > 0) {
            peaks_standard <- rbind(peaks_standard, cbind(
              project_sample = params$project_samples[i],
              peaks2_standard
            ))
          }
