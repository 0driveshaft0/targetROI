...
getR <- function(masses, resmass, nknots = 13, spar = 0.1) {
  ...
  if (any(masses < min(resmass[, 1])) || any(masses > max(resmass[, 1]))) {
    stop(
  ...
  options(digits = 10)
  model <- smooth.spline(resmass[, 1], resmass[, 2],
    cv = TRUE,
...
get_theoric <- function(formulas, charge, resolution = NULL) {
  ...
  if (!is.null(resolution)) {
    ...
    lapply(pattern, get_isotope_annot)
  }
}
...
get_isotope_annot <- function(isotopic_pattern) {
  if (class(isotopic_pattern) == "character") {
    return(data.frame(matrix(,
      nrow = 0, ncol = 3, dimnames = list(
        c(),
        c("mz", "abundance", "iso")
      )
    )))
  ...
  isos <- paste0("A", isos)
  isotopic_pattern <- data.frame(
    mz = isotopic_pattern[, 1],
...
get_patterns_status <- function(patterns, mz_range, adduct = NULL) {
  ...
  for (i in 1:length(patterns)) {
    ...
    if (max(patterns[[i]]["mz"]) < mz_range[1] | min(patterns[[i]]["mz"]) > mz_range[2]) {
      ...
      if (min(reducted_pattern["mz"]) < mz_range[1] | max(reducted_pattern["mz"]) > mz_range[2]) {
        status <- "outside"
...
get_tic <- function(db, sample) {
  ...
  data <- if (all(ms_file@tic == 0)) {
    data.frame(
      rt = ms_file@scantime,
      int = xcms::rawEIC(ms_file, mzrange = range(ms_file@env$mz))
    )
  } else {
    data.frame(rt = ms_file@scantime, int = ms_file@tic)
  }
  rm(ms_file)
  gc()
  # concatenate values (too many in GC-MS) (2 pts per sec)
  rt <- aggregate(data["rt"],
    by = list(rounded = round(data$rt * 2)), FUN = min
  )$rt
  int <- aggregate(data["int"],
    by = list(rounded = round(data$rt * 2)), FUN = max
  )$int
  data.frame(
    rt = rt / 60,
    int = int
...
subform <- function(forms, form) {
  if (form == "FALSE") {
    ...
    if (any(!test)) {
      enviPat::subform(forms[which(!test)], form)
    } else {
      NULL
    }
  }
