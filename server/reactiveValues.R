...
users <- shiny::eventReactive(actualize$users, db_get_query(
  db,
...
#' }
samples <- shiny::eventReactive(actualize$samples, db_get_query(
  db,
...
#' }
projects <- shiny::eventReactive(actualize$projects, db_get_query(
  db,
  "select * from project;"
))
...
#' }
project_samples <- shiny::eventReactive(actualize$project_samples, db_get_query(
  db,
...
shiny::observeEvent(c(project_samples(), input$project), {
  ...
  shinyWidgets::updatePickerInput(session, "files_in_db",
    label = NULL,
    choices = choices, selected = selected
  )
  shiny::updateSelectInput(session, "files_from_db",
    label = NULL,
    choices = choices, selected = selected
  )
  shiny::updateSelectInput(session, "regression_theoretic_files",
    label = "Samples",
    choices = choices
  )
  choices <- project_samples()[
    which(project_samples()$project == input$project),
...
#' }
deconvolution_params <- shiny::eventReactive(
...
shiny::observeEvent(c(deconvolution_params(), input$project), {
  ...
  if ("Standard" %in% names(splitTable)) {
    ...
    shiny::updateSelectInput(session, "process_results_study",
      "Type",
      choices = splitTable, selected = "Standard"
    )
  } else {
  ...
  if (input$process_results_chemical_adduct != "") {
    if (input$process_results_chemical_adduct %in% choices) {
      shiny::updateSelectInput(session, "process_results_chemical_adduct",
        "Adduct",
        choices = as.factor(choices), selected = input$process_results_chemical_adduct
      )
    } else {
      shiny::updateSelectInput(session, "process_results_chemical_adduct",
        "Adduct",
        choices = as.factor(choices), selected = as.factor(choices)[1]
      )
    }
  } else {
    shiny::updateSelectInput(session, "process_results_chemical_adduct",
