...
load_ms_file <- function(db, sampleID) {
  ...
  if (is.null(ms_file)) {
    ...
    if (length(paths) == 0) {
      stop(sprintf("\"%s\" is not found in database", sampleID))
    } else {
...
get_project_polarity <- function(db, project) {
  query <- sprintf(
    "select polarity from sample where sample == (
...
get_project_mz_range <- function(db, project_sample) {
  query <- sprintf(
    "select mz_min, mz_max from sample where sample in (
...
get_samples <- function(db, project = NULL, project_samples = NULL) {
  ...
  } else if (!is.null(project_samples)) {
    query <- sprintf(
...
get_features <- function(db, project_samples = NULL, adducts = NULL, chemical_type = NULL) {
  if (is.null(project_samples) | is.null(adducts)) {
    return(data.frame())
  }
  query <- sprintf(
    "select * from feature where project_sample in (%s)
...
get_chemical_features <- function(db, project_sample = NULL,
                                  chemical_ion = NULL) {
  if (is.null(project_sample) | is.null(chemical_ion)) {
    return(data.frame())
  }
  query <- sprintf(
    "select * from feature where project_sample == %s
...
get_deconvolution_params <- function(db, project, chemical_type, adduct) {
  query <- sprintf(
    "select * from deconvolution_param
...
get_profile_matrix <- function(
    db, project_sample = NULL, adduct = NULL,
  ...
  if (is.null(adduct)) {
    ...
    profile_mat <- matrix(NA,
      nrow = colY[2] - colY[1] + 1, ncol = colX[2] - colX[1] + 1,
      dimnames = list(paste0(colnames(chemicals)[1], colY[1]:colY[2]), paste0(colnames(chemicals)[2], colX[1]:colX[2]))
    )
    if (is.null(project_sample) | is.null(adduct)) {
      return(profile_mat)
    }
    query <- sprintf(
      "SELECT chemical_ion,
    ...
    if (nrow(data) > 0) {
      if (table) {
        return(merge(chemicals, data, by = "chemical_ion", all.x = TRUE))
      }
      ion_forms <- get_chemical_ions(db, adduct, chemical_type)
      theoric_patterns <- get_theoric(ion_forms$ion_formula, ion_forms$charge[1])
      mz_range <- get_project_mz_range(db, project_sample)
...
get_standard_table <- function(db, project = NULL, adduct = NULL, standard_formula = NULL) {
  ...
  for (i in 1:length(sample$project_sample)) {
    ...
    query <- sprintf(
      'select intensities, intensities_b, score, weighted_deviation from feature where
    ...
    if (nrow(data) == 0) {
      data <- cbind(
        sample_id = sample$sample_id[i], formula = standard_formula, adduct = adduct, intensities = "not possible",
